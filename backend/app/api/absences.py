...
def update_absence(
    ...
    absence = db.get_absence_by_id(absence_id, current_user["id"])
    if not absence:
        raise HTTPException(status_code=404, detail="Absence not found")
    
    updated_absence = db.update_absence(absence_id, user_id=current_user["id"], applied=absence_update.applied)
...
def delete_absence(
    ...
    db.delete_absence(absence_id, current_user["id"])
...
def bulk_delete_absences(
    ...
    deleted_count = db.bulk_delete_absences(absence_ids, current_user["id"])
...
def bulk_update_applied(
    ...
    updated_count = db.bulk_update_applied(absence_ids, current_user["id"], applied)
